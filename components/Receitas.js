...
const SeuApp = () => {
  const [id_categoria_entrada, setId_Categoria_Entrada] = useState([]);
  ...
  useEffect(() => {
    fetch('https://apifinanceiropessoal.webapptech.site/api/categoria_entrada')
      .then(res => res.json())
      .then(data => {
        console.log("categorias", data);
        setId_Categoria_Entrada(Array.isArray(data) ? data : data.data || []);
      });
  ...
  const cadastrarReceita = () => {
    fetch('https://apifinanceiropessoal.webapptech.site/api/receitas', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id_categoria_entrada: categoriaSelecionada,
        valor,
        data,
        descricao,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.inputBox}>
        ...
        >
          ...
          {id_categoria_entrada.map((cat) => (
            <Picker.Item key={cat.id_Categoria_entrada}  label={cat.nome} value={cat.id} />
          ))}
        </Picker>
      </View>
